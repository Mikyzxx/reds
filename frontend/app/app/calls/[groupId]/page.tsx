...
export default function CallRoomPage({
  ...
  // avisar al hook cuándo el panel está visible (gobierna los no-leídos);
  // al salir de la página el panel queda cerrado para que cuente de nuevo
  useEffect(() => {
  ...
  return (
    <div className="flex h-full min-w-0 flex-1 flex-col px-6 py-5">
      ...
      <div className="flex min-h-0 flex-1 gap-3">
        ...
        {chatOpen && (
          <ChatPanel
            ...
            onClose={() => setChatOpenLocal(false)}
            onUpload={(file) => uploadAttachment(id, file)}
          />
        )}
      </div>
      ...
      <div className="flex justify-center gap-3 pt-[18px] pb-1">
        ...
        <button
          onClick={() => setChatOpenLocal((v) => !v)}
          className={`relative flex w-24 cursor-pointer items-center justify-center gap-1.5 border py-3 font-mono text-[11px] tracking-wide hover:border-cyan ${
            chatOpen
        ...
        >
          ...
          {chatUnread > 0 && !chatOpen && (
            <span className="absolute -top-1.5 -right-1.5 bg-cyan px-1.5 py-px font-mono text-[9px] font-semibold text-[#04121a]">
              {chatUnread}
            </span>
          )}
        </button>
